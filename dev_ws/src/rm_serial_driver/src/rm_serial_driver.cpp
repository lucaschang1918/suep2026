  ...
  while (rclcpp::ok()) {
    try {
      ...
      if (header[0] == 0x5A) {
        ...
        // RCLCPP_INFO(this->get_logger(), "\n\nrec_detect_color: %u", packet.detect_color);
        // RCLCPP_INFO(this->get_logger(), "rec_rool: %f", packet.roll);
        // RCLCPP_INFO(this->get_logger(), "rec_pitch: %f", packet.pitch);
        // RCLCPP_INFO(this->get_logger(), "rec_yaw: %f\n\n", packet.yaw);
        ...
        if (crc_ok) {
          ...
          RCLCPP_INFO(this->get_logger(), "CRC OKOKOK");
  ...
  const auto_aim_interfaces::msg::TimeInfo::ConstSharedPtr time_info)
{
  ...
  try {
    ...
    // RCLCPP_INFO(this->get_logger(), "\nsend_state: %u", packet.state);
    // RCLCPP_INFO(this->get_logger(), "send_x: %f", packet.x);
    // RCLCPP_INFO(this->get_logger(), "send_y: %f", packet.y);
    // RCLCPP_INFO(this->get_logger(), "send_z: %f\n", packet.z);
