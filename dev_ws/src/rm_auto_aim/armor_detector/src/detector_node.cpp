...
namespace rm_auto_aim
{
        ...
        if (true)
        {
            ...
            if (!armors.empty())
            {
                ...
                number_img_pub_.publish(
                    *cv_bridge::CvImage(
                         img_msg->header, "mono8", all_num_img)
                         .toImageMsg());
            }
    ...
    void ArmorDetectorNode::processingLoop()
    {
        while (rclcpp::ok())
        {
            sensor_msgs::msg::Image::ConstSharedPtr img_copy;

            {
                std::lock_guard<std::mutex> lock(mutex_);
                if (latest_img_)
                {
                    img_copy = latest_img_;
                }
            }

            if (img_copy)
            {
                auto armors = detectArmors(img_copy);

                {
                    std::lock_guard<std::mutex> lock(mutex_);
                    latest_armors_ = armors;         // 即使为空也更新
                    new_detection_available_ = true; // 设置标志
                }
            }
